...
class Track extends Controller {
	...
	public function input() {
		...
		if(isset($_FILES['track_file']) && $_FILES['track_file']['error'] === UPLOAD_ERR_OK) {
			...
			if(move_uploaded_file($file_tmp, $file_dir . $file_name_new)) {
				$sql = "INSERT INTO track (track_name, album_id, track_time, track_file) VALUES (:track_name, :album_id, :track_time, :track_file)";
	...
	{
		$sql = "SELECT tr.*, al.album_name as ALB, ar.artist_name as ART FROM track tr INNER JOIN album al ON tr.album_id=al.album_id LEFT JOIN artist ar ON al.artist_id=ar.artist_id";
		$stmt = $this->db->prepare($sql);
		$stmt->execute();
